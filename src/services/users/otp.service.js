...
import { badRequest, FIELD_ERROR } from "../../errors/error.js";
import { USER_STATUS } from "../../db/models/user/user.js";

import { OTP_CODE_TYPE } from "../../constants/common.constant.js";
import { generateVerifyCode } from "../../utils/helper.util.js";
import { mailAwsService } from "../common/ses.service.js";
import { OTP_STATUS } from "../../db/models/otp.js";
...
export async function checkUserExisted(
  ...
  if (!email && !phone) {
    throw badRequest(
      "register",
      FIELD_ERROR.PHONE_OR_EMAIL_IS_REQUIRED,
  ...
  if (phoneCode && phoneNumber) {
    ...
    if (phoneExisted) {
      throw badRequest(
        "register-user-email",
        FIELD_ERROR.PHONE_NUMBER_IS_USED,
        `Phone ${phone} is used`
      );
    }
  }
  ...
  if (name) {
    ...
    if (nameExisted) {
      throw badRequest(
        "register-username",
        FIELD_ERROR.USER_NAME_IS_EXITED,
        `User name ${name} is existed`
      );
    }
  }
  ...
  if (email && userNameFilter.indexOf(email.trim().toLowerCase()) >= 0) {
    throw badRequest(
      "register-email",
      FIELD_ERROR.EMAIL_NOT_ALLOW_REGISTER,
